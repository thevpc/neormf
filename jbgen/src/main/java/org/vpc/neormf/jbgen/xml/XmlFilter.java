...
public class XmlFilter {
    ...
    public static XmlFilter[] valueOf(String path) {
        return valueOf(path, false, true);
    }

    public static String toString(XmlFilter[] config) {
        StringBuilder sb = new StringBuilder();
        boolean first = true;
        for (int i = 0; i < config.length; i++) {
            XmlFilter configFilter = config[i];
            if (!first) {
                sb.append(".");
            } else {
                first = false;
    ...
    public static XmlFilter[] valueOf(String path, boolean reorder, boolean onlyEnabled) {
        PropertiesUtils.PropEntry[] list = new PropertiesUtils.PropList(path).getEntries();
        XmlFilter[] f = new XmlFilter[list.length];
        for (int i = 0; i < f.length; i++) {
            Map map = list[i].toMap();
            f[i] = new XmlFilter(list[i].getPrefix());
    ...
    public XmlFilter setAttribute(String attrib, String value) {
        return setAttribute(attrib, value, false);
    }

    public XmlFilter setAttribute(String attrib, String value, boolean mandatory) {
        if (value == null) {
            if (attributes != null) {
    ...
    public XmlFilter setBestFirst(boolean bestFirst) {
