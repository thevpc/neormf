 ...
 * @modification on ---- by -----
 */
public class CSharpTimeTypeGen implements CSharpDataTypeGen {
    public String toCode(DataType type) {
        TimeType him = (TimeType) type;
        if (him.getMin() == 0 && him.getMax() == 0) {
            return !him.isNullable() ? (him.getClass().getName() + ".TIME_NON_NULLABLE") : (him.getClass().getName() + ".TIME_NULLABLE");
        }
        return "new " + him.getClass().getName() + "(" + him.isNullable() +
                "," + (him.getMin() <= 0 ? "null" : "new TimeSpan(" + him.getMin() + "L)") +
                "," + (him.getMax() <= 0 ? "null" : "new TimeSpan(" + him.getMax() + "L)") +
                ")";
    }
}
