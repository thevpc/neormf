 ...
 * @modification on ---- by -----
 */
public class OrderListGenerator extends JBGenDAOGenerator {


    public OrderListGenerator(JBGenMain jbgen) {
        super(jbgen);
    }
    ...
    public void generate(Connection connection, DAOInfo entityInfo) throws SQLException, IOException {
        ...
        theClass.setName(entityInfo.getOrderListName());
        theClass.setSuperClass("org.vpc.neormf.commons.beans.OrderList");
...
//            theClass.addField(field);
            ...
            if (getterImpl.getType() == FieldFormulaType.sqlMasterDetail || getterImpl.getType() == FieldFormulaType.sqlDetailMaster) {
                //do not generate !! not supported
            } else {
                theClass.addMethod(new JavaMethod(JavaUtils.method(column, "add"), theClass.getName(), null, "public", null, null,
                        "super.add(" + fieldName + ");\n" +
                                "return this;"));
                theClass.addMethod(new JavaMethod(JavaUtils.method(column, "add"), theClass.getName(), new JavaParam[]{new JavaParam("asc", "boolean", null)}, "public", null, null,
                        "super.add(" + fieldName + ",asc);\n" +
                                "return this;"));
        ...
        entityInfo.setGeneratedClass("OrderList", theClass);
//        JBGenUtils.askFileReadOnly(theClass.getFile(destFolder));
        try {
            if (theClass.rewrite(destFolder,getLog())) {
                getLog().info(" generating OrderList Class " + theClass.getPackage() + "." + theClass.getName() + " to " + destFolder.getCanonicalPath() + " ...");
            }
