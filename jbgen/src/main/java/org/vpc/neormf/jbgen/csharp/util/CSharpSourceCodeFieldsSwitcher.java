 ...
 * @modification on ---- by -----
 */
public abstract class CSharpSourceCodeFieldsSwitcher {
    ...
    public static final String ITERATE_MAP =
            "String selectedFieldName=(String)selectedEntry.Key;" +
            "Object selectedFieldValue=(Object)selectedEntry.Value;"
            ;
    ...
    protected CSharpSourceCodeFieldsSwitcher(String collectionExpression, String iterateTypeCode, boolean autoBreak, DBColumn[] columns) {
...
//            System.out.println(column.getTableName()+"."+column.getColumnName());
    ...
    public String getCode0() {
        ...
        corps.append("for(IEnumerator i=").append(iteratorExpression).append(";i.MoveNext();){\n");
        corps.append(JBGenUtils.indent(getFieldNameMapEntryCastCode()));
        corps.append("\n");
        corps.append("  int selectedFieldId=selectedFieldName.GetHashCode();\n");
...
//                    corps.append("\n");
    ...
    public String getCode() {
        ...
        corps.append(JBGenUtils.indent(getFieldNameMapEntryCastCode()));
        corps.append("\n");
        corps.append("  switch(selectedFieldName){\n");
        for (Iterator i = getHashCodeMap().entrySet().iterator(); i.hasNext();) {
            Map.Entry entry = (Map.Entry) i.next();
            int hashCode = ((Integer) entry.getKey()).intValue();
            HashEntry hashEntry = (HashEntry) entry.getValue();
            ...
            for (int j = 0; j < v.size(); j++) {
                DBColumn c = (DBColumn) v.get(j);
                if(!c.getColumnName().equals(c.getBeanFieldName())){
                    corps.append("    case \"").append(c.getColumnName()).append("\":\n");
                }
                corps.append("    case \"").append(c.getBeanFieldName()).append("\":{");
                if (hashEntry.types.get(j).equals(PRIVATE_COL_TYPE_FIELD)) {
                    corps.append("  //field ").append(c.getBeanFieldName()).append("\n");
                } else if (hashEntry.types.get(j).equals(PRIVATE_COL_TYPE_COLUMN)) {
                    corps.append("  //column ").append(c.getColumnName()).append("\n");
